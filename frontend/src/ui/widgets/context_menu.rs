    ...
    Theme: Catalog,
{
    ...
    class: Theme::Class<'a>,
    /// Force the menu to be shown (for testing purposes). If None, uses
    /// internal state.
    ...
    Theme: Catalog,
{
    ...
    ///
    /// `overlay`: The content of [`ContextMenuOverlay`] which will be displayed
    /// when `underlay` is clicked.
    ...
    Theme: Catalog,
{
    ...
    fn overlay<'b>(
        ...
        viewport: &Rectangle,
        translation: Vector,
    ) -> std::option::Option<iced::advanced::overlay::Element<'b, Message, Theme, Renderer>> {
...
#[cfg(test)]
mod tests {
    use iced::{Renderer, Theme, advanced::widget::text};

use self as context_menu;
...
use super::*;
    ...
    fn create_overlay() -> Element<'static, TestMessage, Theme, Renderer>
    {
        text::Text::new("Overlay").into()
    }
    ...
    fn context_menu_new_creates_instance() {
        let underlay = text::Text::new("Underlay");
        let context_menu = TestContextMenu::new(underlay, create_overlay);

        let size =
            Widget::<TestMessage, Theme, Renderer>::size(&context_menu);
    ...
    fn context_menu_tag_returns_state_tag() {
        ...
        let tag =
            Widget::<TestMessage, Theme, Renderer>::tag(&context_menu);
    ...
    fn context_menu_has_two_children() {
        ...
        let children = {
            let mut tree = Tree::new(
                &context_menu
                    as &dyn Widget<TestMessage, Theme, Renderer>,
            );
    ...
    fn widget_state_creates_correct_initial_state() {
        ...
        let state =
            Widget::<TestMessage, Theme, Renderer>::state(&context_menu);
    ...
    fn open_method_can_be_chained() {
        ...
        let context_menu = TestContextMenu::new(underlay, create_overlay)
            .open(true)
            .class(<Theme as context_menu::Catalog>::default());
